...
BEGIN
    ...
    BEGIN TRY
		...
		SELECT
			...
			CASE 
				WHEN UPPER(TRIM(cst_marital_status)) = 'S' 
					AND UPPER(TRIM(cst_gndr)) = 'F' THEN N'Не замужем'
				WHEN UPPER(TRIM(cst_marital_status)) = 'S' 
					AND UPPER(TRIM(cst_gndr)) = 'M' THEN N'Холост'
				WHEN UPPER(TRIM(cst_marital_status)) = 'M' 
					AND UPPER(TRIM(cst_gndr)) = 'F' THEN N'Замужем'
				WHEN UPPER(TRIM(cst_marital_status)) = 'M' 
					AND UPPER(TRIM(cst_gndr)) = 'M' THEN N'Женат'
				ELSE N'Неизвестно'
			END AS cst_marital_status, -- Normalize marital status 
			CASE 
				WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN N'Женщина'
				WHEN UPPER(TRIM(cst_gndr)) = 'M' THEN N'Мужчина'
				ELSE N'Неизвестно'
		...
		WHERE rank_flag = 1; -- Select the most recent record per customer
		...
		SELECT
			...
			END AS cid, 
			CASE
				WHEN bdate > GETDATE() THEN NULL -- Remove future bithdays
				WHEN bdate < '1920-01-01' THEN NULL
				ELSE bdate
			END AS bdate, 
			CASE
				WHEN UPPER(TRIM(gen)) IN ('F', 'FEMALE') THEN N'Женщина'
				WHEN UPPER(TRIM(gen)) IN ('M', 'MALE') THEN N'Мужчина'
				ELSE 'Неизвестно'
